...
public interface LessonRepository extends JpaRepository<Lesson, Long> {
    ...
    @Query("""
        ...
        WHERE s.idSt.id = :idSt
            AND s.idGroup.id = :idGroup
            AND (s.subgroup.id IS NULL OR s.subgroup.id = :idTeacher)
    """)
    List<LessonInfoDTO> findLessonInfo(@Param("idSt") Long idSt,
                                       @Param("idGroup") Long idGroup,
                                       @Param("idTeacher") Long idTeacher);

    @Query("""
        SELECT DISTINCT new org.spring.diaryBackend.dto.other.LessonDateDTO(
            rm.id.number, l.id, l.date
        )
        FROM Schedule s
        JOIN s.lessons l
    ...
    """)
    List<LessonDateDTO> findLessonDates(@Param("idSt") Long idSt,
                                        @Param("idGroup") Long idGroup,
                                        @Param("idTeacher") Long idTeacher);
    ...
    @Query(nativeQuery = true,
           ...
           """)
    void updateLessonSupplement(@Param("id_supplement") Long idSupplement);
}