...
public class SimpleSubgroupService implements SubgroupService {
    ...
    public void save(SubgroupDTO subgroupDTO) {
        ...
        subgroup.setStudents(
                subgroupDTO.getStudents()
                        .stream()
                        .map(student -> studentRepository.findById(student).orElse(null))
    ...
    public void addStudents(SubgroupStudentsDTO subgroupStudentsDTO) {
        Subgroup subgroup = subgroupRepository.findTeacherSubgroupBySubject(
                subgroupStudentsDTO.getIdSt(),
                subgroupStudentsDTO.getIdTeacher()
        );
        List<Long> students = new java.util.ArrayList<>(
                Objects.requireNonNull(subgroup).getStudents()
                        .stream()
                        .map(Student::getId)
                        .toList()
        );
    ...
    public void updateStudents(SubgroupStudentsDTO subgroupStudentsDTO, Long idSt, Long idTeacher) {
        Subgroup subgroupOne = subgroupRepository.findTeacherSubgroupBySubject(
                subgroupStudentsDTO.getIdSt(),
                subgroupStudentsDTO.getIdTeacher()
        );
        Subgroup subgroupTwo = subgroupRepository.findTeacherSubgroupBySubject(idSt, idTeacher);

        List<Long> studentsSubgroupTwo = new java.util.ArrayList<>(
                Objects.requireNonNull(subgroupTwo).getStudents()
                        .stream()
                        .map(Student::getId)
                        .toList()
        );
        studentsSubgroupTwo.addAll(subgroupStudentsDTO.getStudents());
        subgroupTwo.setStudents(
                studentsSubgroupTwo.stream()
                        .map(student -> studentRepository.findById(student).orElse(null))
                        .collect(Collectors.toSet())
        );
        subgroupRepository.save(subgroupTwo);

        List<Long> studentsSubgroupOne = new java.util.ArrayList<>(
                Objects.requireNonNull(subgroupOne).getStudents()
                        .stream()
                        .map(Student::getId)
                        .toList()
        );
        studentsSubgroupOne.removeAll(subgroupStudentsDTO.getStudents());
        subgroupOne.setStudents(
                studentsSubgroupOne.stream()
                        .map(student -> studentRepository.findById(student).orElse(null))
                        .collect(Collectors.toSet())
        );
